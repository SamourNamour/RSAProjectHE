...
namespace MTV.Catalog.Host
{
    ...
    public class MEBSCatalog : DataService<mebsEntities>
    {
        public static void InitializeService(DataServiceConfiguration config)
        {
            ...
            config.SetServiceOperationAccessRule("AcquireDummyCommandLock", ServiceOperationRights.AllRead);
            config.SetServiceOperationAccessRule("AcquirePosterLock", ServiceOperationRights.AllRead);
            config.SetServiceOperationAccessRule("Ping", ServiceOperationRights.AllRead);
            config.SetServiceOperationAccessRule("PingMySQLService", ServiceOperationRights.AllRead);
        ...
        public IQueryable<mebs_ingesta> GetPackagesToSchedule(int Type)
        {
            try
            {
                //------ Get only liste of Video (DC or Advertisement depond on the Type)

                if (Type != 1) return null;

                return (from DCItem in this.CurrentDataSource.mebs_ingesta
                        join IC in this.CurrentDataSource.mebs_ingesta_category_mapping on DCItem.IdIngesta equals IC.IdIngesta
                        where DCItem.Type == Type && DCItem.IsExpired == false
                        && DCItem.MediaFileSizeAfterRedundancy != 0
                        && DCItem.Poster != null
                        orderby DCItem.Date_Creation descending
                        select DCItem).Distinct();
            }
            catch (Exception ex)
            {
                throw new ApplicationException("An error occured: {0}", ex);
        ...
        /// <returns></returns>
        [WebGet]
        public bool AcquireDummyCommandLock(int IDSchedule)
        {
            ...
            if (scheduleEvent.Dummy_Status != Convert.ToInt32(EpgDummyCommandStatus.PREPARED))
                return false;

            scheduleEvent.Dummy_Status = Convert.ToInt32(EpgDummyCommandStatus.TOKEN_OWNED);
            this.CurrentDataSource.SaveChanges();

            scheduleEvent = this.CurrentDataSource.mebs_schedule.ToList().Where(item => item.IdSchedule == IDSchedule).FirstOrDefault();

            return (scheduleEvent.Dummy_Status == Convert.ToInt32(EpgDummyCommandStatus.TOKEN_OWNED) ?
                    true :
                    false);
        }

        [WebGet]
        public bool AcquirePosterLock(int IDSchedule)
        {
            ...
            if (scheduleEvent.Poster_Status != Convert.ToInt32(PosterTransferStatus.PREPARED))
                return false;

            scheduleEvent.Poster_Status = Convert.ToInt32(PosterTransferStatus.TOKEN_OWNED);
            this.CurrentDataSource.SaveChanges();

            scheduleEvent = this.CurrentDataSource.mebs_schedule.ToList().Where(item => item.IdSchedule == IDSchedule).FirstOrDefault();

            return (scheduleEvent.Poster_Status == Convert.ToInt32(PosterTransferStatus.TOKEN_OWNED) ?
                    true :
                    false);
        }
        #endregion
        ...
        [WebGet]
        public string PingMySQLService()
        {
            try
            {
                this.CurrentDataSource.DatabaseExists();
                return "ACK";
            }
            catch (Exception ex)
            {
