...
namespace MTV.Scheduler.App.MTVControl
{
    ...
    public class WindowsServiceManager
    {
            ...
            foreach (ServiceController service in services)
            {
                if (String.Compare(service.ServiceName, serviceName, true) == 0)
                {
                    if (service.Status == ServiceControllerStatus.Stopped)
                    {
                        ...
                        while (!IsServiceRunning(serviceName) && hackCounter < 60)
                        {
        ...
        /// <summary>
        /// Start/Stop cycles a given Service
        /// </summary>
        /// <returns>Always true</returns>
        public bool Restart(string aServiceName)
        {
            int hackCounter = 0;
            if (!IsServiceInstalled(aServiceName)) return false;

            StopService(aServiceName);

            while (!IsStopped(aServiceName) && hackCounter < 120) // wait a maximum of 30 seconds
            {
                System.Threading.Thread.Sleep(250);
                hackCounter++;
            }
            ...
            while (!IsServiceRunning(aServiceName) && hackCounter < 60)
            {
                System.Threading.Thread.Sleep(250);
                hackCounter++;
            }
        ...
        /// <summary>
        /// Is the status of a given Service == Stopped
        /// </summary>
        public bool IsStopped(string aServiceName)
        {
